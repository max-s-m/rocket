...
def gen_type_conv(conversion_type):
    """Генерує інструкції ПОЛІЗ для конвертації типів на стеку."""
    if conversion_type == 'i2f_r':
        poliz.postfix_code_gen('i2f', 'conv')
    elif conversion_type == 'i2f_l':
        poliz.postfix_code_gen('SWAP', 'stack_op')
        poliz.postfix_code_gen('i2f', 'conv')
        poliz.postfix_code_gen('SWAP', 'stack_op')
    elif conversion_type == 'i2f_both':
        poliz.postfix_code_gen('i2f', 'conv')  # R -> R_f
        poliz.postfix_code_gen('SWAP', 'stack_op')
        poliz.postfix_code_gen('i2f', 'conv')  # L -> L_f
        poliz.postfix_code_gen('SWAP', 'stack_op')  # Порядок [L_f, R_f]
...
def parseDeclaration(kind='variable'):
    ...
    if numRow <= len_tableOfSymb and getSymb()[1] == '=':
        ...
        if declared_type == 'int' and expr_type == 'float':
            poliz.postfix_code_gen('f2i', 'conv')
...
def parseReturnStatement():
    ...
    if getSymb()[1] == ';':
        ...
        if expected_type == 'int' and expr_type == 'float':
            poliz.postfix_code_gen('f2i', 'conv')
...
def parseAssign():
    ...
    if getSymb()[1] in input_map:
        ...
        if input_func == 'inputInt':
            poliz.postfix_code_gen('s2i', 'conv')
        elif input_func == 'inputFloat':
            poliz.postfix_code_gen('s2f', 'conv')
        elif input_func == 'inputBool':
            poliz.postfix_code_gen('s2i', 'conv')
            poliz.postfix_code_gen('i2b', 'conv')
    else:
        expr_type = parseExpression()
        if assign_lex == '=':
            ...
            result_type, conv = semant.check_arithm_op(id_type, arith_op, expr_type, assign_line)
            gen_type_conv(conv)
            semant.check_assign(id_type, result_type, assign_line)
            poliz.postfix_code_gen(arith_op, 'math_op')
            if id_type == 'int' and result_type == 'float':
                poliz.postfix_code_gen('f2i', 'conv')
...
def parseIf():
    ...
    # --- ВИПРАВЛЕНО: Додаємо мітку перед двокрапкою ---
...
def parseFor():
    ...
    # --- ВИПРАВЛЕНО ---
    poliz.set_label_value(label_incr)
    poliz.current_rpn_table.append(label_incr)
    poliz.postfix_code_gen(':', 'colon')
...
def parseSwitch():
    ...
    poliz.current_rpn_table.append(label_end_switch)
    poliz.postfix_code_gen(':', 'colon')
...
def parseFunctionDeclaration():
    ...
    global numRow
    parent_rpn = poliz.current_rpn_table
    parent_labels = poliz.current_label_table
    func_rpn = []
    func_labels = {}
    poliz.current_rpn_table = func_rpn
    poliz.current_label_table = func_labels
    ...
    if current_func['return_type'] == 'void' and (not func_rpn or func_rpn[-1] != ('RET', 'RET')):
        poliz.postfix_code_gen('RET', 'RET')

    current_func_filename = f"{base_file_name}${func_name}"
    funcs_scope = semant.tabName['univ']
    poliz.generate_postfix_file(
        current_func_filename,
        semant.tabName[semant.currentContext],
        func_rpn,
        func_labels,
        funcs_scope
    )
    ...
    poliz.current_rpn_table = parent_rpn
    poliz.current_label_table = parent_labels
    predIndt()
...
def parseOr():
    ...
    while numRow <= len_tableOfSymb and getSymb()[1] == '||':
        ...
        poliz.postfix_code_gen('OR', 'bool_op')
    predIndt()
    return l_type
...
def parseAnd():
    ...
    while numRow <= len_tableOfSymb and getSymb()[1] == '&&':
        ...
        r_type = parseRel()
        l_type = semant.check_logic_op(l_type, lex, r_type, line)
        poliz.postfix_code_gen('AND', 'bool_op')
    predIndt()
    return l_type
...
def parseRel():
    ...
    if numRow <= len_tableOfSymb and getSymb()[2] == 'rel_op':
        ...
        res_type, conv = semant.check_rel_op(l_type, lex, r_type, line)
        gen_type_conv(conv)
...
def parseArithmExpression():
    ...
    while numRow <= len_tableOfSymb and getSymb()[2] == 'add_op':
        ...
        numRow += 1
        r_type = parseTerm()
        l_type, conv = semant.check_arithm_op(l_type, lex, r_type, line)
        gen_type_conv(conv)
        poliz.postfix_code_gen(lex, tok)
    predIndt()
    return l_type
...
def parseTerm():
    ...
    while numRow <= len_tableOfSymb and getSymb()[2] == 'mult_op':
        ...
        numRow += 1
        r_type = parsePower()
        l_type, conv = semant.check_arithm_op(l_type, op_lex, r_type, op_line)
        gen_type_conv(conv)
...
def parseFactor():
    ...
    if (lex, tok) in (('+', 'add_op'), ('-', 'add_op')):
        ...
        factor_type = 'bool'
        poliz.postfix_code_gen('NOT', 'bool_op')
    else:
...
if FSuccess == ('Rocket', True):
    try:
        ...
        # print("\nFinal Symbol Table:")
        # import json
        # def default_serializer(o): return str(o)
